...
async function guardarEnFirestore(user, extra = {}) {
  if (!user || !user.uid) throw new Error('Usuario no válido');
  ...
  try {
    ...
    console.error('Error en guardarEnFirestore:', error);
    throw error;
  }
}
...
async function cargarPerfil(user) {
  if (!user || !user.uid) {
    console.warn('cargarPerfil llamado sin usuario');
    return;
  }
  try {
    const ref = doc(db, 'usuarios', user.uid);
    ...
    if (profileNameDisplay) profileNameDisplay.textContent = nombre;
    if (profileAvatar) {
    ...
    if (typeof showToast === 'function') {
      showToast('❌ Error al cargar perfil. Revisa tu conexión.', true);
    }
    throw error;
  }
}
...
function renderAvatars(selected) {
  ...
  DEFAULT_AVATARS.forEach(url => {
    ...
    img.onclick = () => {
      ...
      if (modalProfileImage) modalProfileImage.src = url;
...
async function guardarPerfil(nuevoNombre, nuevaFoto) {
  ...
  try {
    ...
    const profileNameDisplay = document.getElementById('profileNameDisplay');
    const profileAvatar = document.getElementById('profileAvatar');
    if (userNameSpan) userNameSpan.textContent = nuevoNombre || user.displayName;
    if (profileNameDisplay) profileNameDisplay.textContent = nuevoNombre || user.displayName;
    if (profileAvatar && fotoURL) {
      profileAvatar.src = fotoURL;
      profileAvatar.onerror = () => profileAvatar.src = FALLBACK;
    }
...
// ============================================================
//  DOMContentLoaded – Todo el código que usa el DOM
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Toast (función global)
  window.showToast = function(msg, isError = false, duration = 5000) {
  ...
  // Hacer accesible globalmente para funciones externas
  ...
  document.getElementById('doLoginBtn').onclick = async () => {
    ...
    try {
      ...
      showToast('❌ Credenciales incorrectas o error de red', true);
    }
  };
  ...
  document.getElementById('editProfileBtn').onclick = async () => {
    ...
    try {
      ...
      showToast('❌ Error al cargar datos del perfil', true);
    }
  };
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      try {
        ...
        console.error('Error al cargar perfil en onAuthStateChanged:', error);
        // El toast ya se muestra dentro de cargarPerfil
      }
    } else {
  ...
  if (track && slides.length) {
    function getSlideWidth() {
      if (wrapper) return wrapper.getBoundingClientRect().width;
    ...
    function updateCarousel(index, animate = true) {
      ...
      else track.style.transition = 'transform 0.7s cubic-bezier(0.68, -0.55, 0.265, 1.55)';
      ...
      currentIndex = index;
      if (!animate) { track.offsetHeight; track.style.transition = 'transform 0.7s cubic-bezier(0.68, -0.55, 0.265, 1.55)'; }
      setTimeout(() => isTransitioning = false, 800);
    }

    function goToNext() { if (!isTransitioning) { updateCarousel(currentIndex + 1); resetAutoPlay(); } }
    function goToPrev() { if (!isTransitioning) { updateCarousel(currentIndex - 1); resetAutoPlay(); } }
    ...
    dots.forEach((dot, index) => {
      dot.addEventListener('click', (e) => {
        e.stopPropagation();
        if (!isTransitioning && index !== currentIndex) { updateCarousel(index); resetAutoPlay(); }
      });
    });

    slides.forEach((slide) => {
      slide.addEventListener('click', (e) => {
        if (e.target.closest('.carousel-nav-btn') || e.target.closest('.indicator-dot')) return;
        const url = slide.dataset.url;
        if (url) {
          slide.style.transform = 'scale(0.95)';
          slide.style.transition = 'transform 0.2s ease';
          setTimeout(() => { slide.style.transform = ''; window.location.href = url; }, 250);
        }
      });
    });
    ...
    function startAutoPlay() {
      ...
      autoPlayInterval = setInterval(() => { if (!isTransitioning) goToNext(); }, 5000);
    }
    ...
    if (wrapper) {
      wrapper.addEventListener('touchstart', (e) => { touchStartX = e.changedTouches[0].screenX; isSwiping = true; }, { passive: true });
      wrapper.addEventListener('touchmove', (e) => { if (isSwiping) e.preventDefault(); }, { passive: false });
      wrapper.addEventListener('touchend', (e) => {
        if (!isSwiping) return;
        touchEndX = e.changedTouches[0].screenX;
        const diff = touchStartX - touchEndX;
        if (Math.abs(diff) > 50) {
          if (diff > 0 && !isTransitioning) goToNext();
          else if (diff < 0 && !isTransitioning) goToPrev();
        }
        isSwiping = false;
      }, { passive: true });
    }
    ...
    function initializeCarousel() {
      ...
      if (menu && menu.style.display !== 'none') {
        setTimeout(() => {
          slideWidth = getSlideWidth();
          if (slideWidth > 0) { updateCarousel(0, false); setTimeout(startAutoPlay, 1500); }
          else setTimeout(initializeCarousel, 300);
        }, 100);
      } else {
        ...
        observer.observe(document.getElementById('menuSection'), { attributes: true, attributeFilter: ['style'] });
      }
    }
  ...
  function createParticles() {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      particle.style.cssText = `position:absolute;width:${size}px;height:${size}px;background:radial-gradient(circle,rgba(255,255,255,0.6),transparent);border-radius:50%;left:${Math.random()*100}%;animation:floatParticle ${duration}s linear infinite;animation-delay:${delay}s;opacity:${opacity};`;
      container.appendChild(particle);
    }
    if (!document.getElementById('particle-styles')) {
      const style = document.createElement('style');
      style.id = 'particle-styles';
      style.textContent = `@keyframes floatParticle{0%{transform:translateY(100vh) rotate(0deg) scale(1);opacity:0}10%{opacity:1}90%{opacity:1}100%{transform:translateY(-10vh) rotate(720deg) scale(0.5);opacity:0}}`;
      document.head.appendChild(style);
    }
  }
  const menuForParticles = document.getElementById('menuSection');
  if (menuForParticles) {
    if (menuForParticles.style.display !== 'none') createParticles();
    else {
      const observer = new MutationObserver(() => {
        if (menuForParticles.style.display !== 'none') { createParticles(); observer.disconnect(); }
      });
      observer.observe(menuForParticles, { attributes: true, attributeFilter: ['style'] });
    }
  }
